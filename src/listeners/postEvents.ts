...
import { popState, updateState, replaceState, replaceNavigation } from "../services/navigationService.ts";
import navigationService from "../services/navigationService.ts";
import { urlRegex, dateRegex, timeRegex } from "../views/utils/data.js";
import { ListenerRequest, Api } from "@lenra/app";
const { getNavigation } = navigationService;

export async function savePost(_props: ListenerRequest['props'], event: { value: { date: number, time: number } }, api: Api) {
    const navigation = await getNavigation(api);
    let post: Post = navigation.state?.post ? await api.data.coll(Post).getDoc(navigation.state.post) : new Post();
    const errors = [];
    // update fields
...
export async function savePostStat(_props: ListenerRequest['props'], event: ListenerRequest['event'], api: Api) {
    const navigation = await getNavigation(api);
